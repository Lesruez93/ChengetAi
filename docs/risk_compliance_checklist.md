...
(minimum safeguards). Narrative reasoning for each item is in
`proposal/ChengetAI_AI4I_Proposal_Development.md` §4 and `docs/dataset_statement.md`; this page is
the checklist form judges expect as a separate artifact.
...
| **Encryption** | Partial | Supabase provides encryption in transit (TLS) and at rest by default on its managed Postgres; this has not yet been independently verified against a live production project (no live Supabase project is exercised in this repo yet — see README "Known limitations"). |
...
1. No live Supabase project has been exercised in this repo — RLS policies are implemented and
   reviewable in `sample_data/seed.sql` but not yet tested against a running production instance.
2. Formal bias/fairness audit against real (non-synthetic) demographic data has not been run.
3. Number-lookup endpoint has no dedicated rate limit or scraping protection yet.
4. Dispute/human-review flow for flagged numbers is designed (proposal §2.6) but not implemented.
