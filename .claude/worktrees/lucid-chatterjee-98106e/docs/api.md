...
`country` also determines the currency reported on every flagged row. One
upload is one market, so `currency_code` and `currency_symbol` are the same for
every entry in `flagged` — they are carried per row so a client can render an
amount without tracking which run it came from. An empty or unrecognised
`country` is accepted and returns `""` for both, meaning "unlabelled": clients
should then show the bare number rather than assume a currency.

> **Caveat.** `currency_*` governs *display* only. The structuring rule still
> compares against a single currency-agnostic figure (`STRUCTURING_THRESHOLD`,
> 500), so in a market whose unit is far smaller than a dollar that threshold is
> not yet economically meaningful. Making the thresholds per-currency is a
> detection change, tracked separately from this labelling fix.
...
```json
{
  ...
  "flagged": [
    { "transaction_id": "TX00042", "timestamp": "...", "agent_id": "AGT-2201",
      "customer_msisdn": "08031234567", "type": "cash_out", "amount": 495000.0,
