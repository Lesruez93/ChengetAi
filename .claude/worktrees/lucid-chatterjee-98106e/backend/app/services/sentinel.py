...
def _rule_reasons(df: pd.DataFrame, currency_symbol: str = "") -> dict[int, list[str]]:
        ...
        if len(group) >= 3:
            ...
            if span <= pd.Timedelta(hours=6):
                for i in group.index:
                    reasons[i].append(
                        f"{len(group)} transactions just under "
                        f"{currency_symbol}{STRUCTURING_THRESHOLD:.0f} "
...
def analyze_transactions(
    raw_bytes: bytes, currency_symbol: str = ""
) -> tuple[pd.DataFrame, np.ndarray, dict[int, list[str]]]:
    """Score and explain a transaction file.

    `currency_symbol` only affects how the structuring reason renders its
    threshold — the detection itself is unchanged, since the thresholds here
    are still a single currency-agnostic figure. See the caveat in
    `docs/api.md` about what that means for non-USD markets.
    """
    df = _load_dataframe(raw_bytes)
    scores = _isolation_forest_scores(df)
    reasons = _rule_reasons(df, currency_symbol)
