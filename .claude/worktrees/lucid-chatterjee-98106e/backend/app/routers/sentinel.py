...
async def analyze(
    ...
    country: str = Form(
        ...
        description="ISO 3166-1 alpha-2 code of the agent network the file came from. "
                    "Recorded on the job for reporting, and used to label every "
                    "flagged amount with that market's currency. The detection rules "
                    "themselves are currency- and market-agnostic. An empty or "
                    "unrecognised code is accepted: amounts then come back unlabelled "
                    "rather than being attributed to a currency we were not told.",
    ),
    ...
    try:
        market = get_country(country_code)
        currency_code, currency_symbol = market.currency_code, market.currency_symbol
    except UnknownCountryError:
        currency_code, currency_symbol = "", ""

    try:
        df, scores, rule_reasons = analyze_transactions(raw_bytes, currency_symbol)
