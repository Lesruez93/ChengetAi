...
def test_sentinel_analyze_labels_flagged_amounts_with_the_markets_currency(client):
    """A Nigerian run must come back in naira, not the "$" this used to assume."""
    with open(SAMPLE_CSV, "rb") as f:
        resp = client.post(
            "/sentinel/analyze",
    ...
    assert flagged
    assert all(t["currency_code"] == "NGN" for t in flagged)
    assert all(t["currency_symbol"] == "₦" for t in flagged)


def test_sentinel_analyze_currency_follows_the_country_form_field(client):
    """Each supported market gets its own currency, not a single global default."""
    for country_code, expected_code, expected_symbol in (
        ("KE", "KES", "KSh"),
        ("ZW", "USD", "$"),
        ("GH", "GHS", "GH₵"),
    ):
        ...
        assert resp.status_code == 200, country_code
        flagged = resp.json()["flagged"]
        assert flagged, country_code
        assert all(t["currency_code"] == expected_code for t in flagged), country_code
        assert all(t["currency_symbol"] == expected_symbol for t in flagged), country_code


def test_sentinel_analyze_returns_no_currency_when_country_is_absent_or_unknown(client):
    """Omitting the country still succeeds; amounts come back unlabelled rather
    than silently attributed to a currency the caller never named."""
    for data in ({}, {"country": "XX"}):
        with open(SAMPLE_CSV, "rb") as f:
            resp = client.post(
                "/sentinel/analyze",
                files={"file": ("transactions_sample.csv", f, "text/csv")},
        ...
        flagged = resp.json()["flagged"]
        assert flagged, data
        assert all(t["currency_code"] == "" for t in flagged), data
        assert all(t["currency_symbol"] == "" for t in flagged), data
