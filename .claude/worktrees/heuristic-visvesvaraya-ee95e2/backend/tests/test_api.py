...
def test_sentinel_analyze_accepts_the_market_the_file_came_from(client):
    with open(SAMPLE_CSV, "rb") as f:
        resp = client.post(
            "/sentinel/analyze",
    ...
    assert resp.status_code == 200
    assert resp.json()["n_flagged"] > 0


def test_sentinel_analyze_rejects_an_unsupported_country(client):
    """The country picks the currency amounts are judged against, so a code we
    don't know would silently analyse the file against the wrong thresholds."""
    with open(SAMPLE_CSV, "rb") as f:
        resp = client.post(
            "/sentinel/analyze",
            files={"file": ("transactions_sample.csv", f, "text/csv")},
