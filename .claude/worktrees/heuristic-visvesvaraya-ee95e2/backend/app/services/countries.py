...
# The round figure that reads as "a large cash amount" in each currency — the
# number a customer splits payments to stay beneath. Keyed by currency rather
# than country because that is what it is a property of: two markets sharing a
# currency share the figure.
#
# These are demo-grade round numbers of roughly comparable real value (~USD
# 500), not any jurisdiction's statutory cash-reporting limit; structuring
# hides under whatever number the operator actually reports at, so a deployment
# should substitute its own network's limits. USD keeps 500, the value the
# Sentinel used when it was USD-only.
#
# Consumed by `app/services/sentinel.py` (which owns how *close* to the figure
# counts as suspicious) and by `sample_data/generate_transactions.py`.
LARGE_AMOUNT_BY_CURRENCY: dict[str, float] = {
    "USD": 500.0,
    "ZAR": 10_000.0,
    "KES": 50_000.0,
    "GHS": 5_000.0,
...
def large_amount_for(code: str | None) -> float:
    """What counts as a large cash amount in this market, in its own currency.

    Falls back to the USD figure for an absent or unrecognised code: callers
    pass an optional, operator-supplied label, and a typo there should degrade
    to a default rather than fail the whole operation.
    """
    country = COUNTRIES.get((code or "").strip().upper())
    currency = country.currency_code if country else DEFAULT_CURRENCY
    return LARGE_AMOUNT_BY_CURRENCY.get(currency, LARGE_AMOUNT_BY_CURRENCY[DEFAULT_CURRENCY])


def currency_symbol_for(code: str | None) -> str:
    country = COUNTRIES.get((code or "").strip().upper())
    if country is not None:
        return country.currency_symbol
    return next(
        (c.currency_symbol for c in COUNTRIES.values() if c.currency_code == DEFAULT_CURRENCY), ""
    )
