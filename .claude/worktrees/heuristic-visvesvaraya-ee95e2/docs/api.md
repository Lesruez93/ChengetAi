...
Agent Fraud Sentinel B2B module. Optional form field `country` (ISO 3166-1
alpha-2, must be a supported market or the request is rejected `422`) tags the
run *and* selects the currency amounts are judged against: the structuring
threshold is per currency, because a threshold meaningful in USD or rand is
trivial in naira or shillings. Omitted, the USD thresholds apply.

Rapid-reversal and unusual-hour detection are genuinely currency-agnostic and
behave the same in every market.

Required columns: `transaction_id, timestamp, agent_id, customer_msisdn, type,
amount` (optional: `is_reversal`, `label`, and `country` — a per-row market
that overrides the form field, for aggregator exports spanning several
markets). See `sample_data/transactions_sample.csv` for the expected shape.
...
```json
{
  ...
  "n_flagged": 20,
  "flagged": [
    { "transaction_id": "TX00042", "timestamp": "...", "agent_id": "AGT-NG-01",
      "customer_msisdn": "+2348012345678", "type": "cash_out", "amount": 495000.0,
      "anomaly_score": 0.71, "reasons": ["5 transactions just under ₦500,000 from the same customer ..."] }
  ],
  "summary_by_reason": { "5 transactions just under ₦500,000 from the same customer": 5 }
}
```
