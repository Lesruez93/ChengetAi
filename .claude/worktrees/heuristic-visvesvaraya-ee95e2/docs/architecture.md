...
2. **Agent Fraud Sentinel** (`services/sentinel.py`) — an `IsolationForest`
   ...
   *Amounts are read per market.* The structuring threshold is keyed off the
   country registry's `currency_code` and the forest sees `amount / threshold`
   rather than a raw amount, because "a large transaction" has no
   currency-free meaning across our markets: a flat threshold meaningful in
   USD switches structuring detection off entirely in naira or shillings, and
   a mixed-currency file would otherwise rank every high-denomination row as
   an outlier on the exchange rate alone.
