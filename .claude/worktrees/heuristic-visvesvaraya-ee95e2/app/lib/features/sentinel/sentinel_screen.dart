...
class _SentinelScreenState extends State<SentinelScreen> {
  ...
  Future<void> _analyze() async {
    ...
    try {
      // The market is recorded on the job *and* picks the currency the backend
      // judges amounts against: a structuring threshold meaningful in USD is
      // trivial in naira. Rows whose CSV carries its own `country` column keep
      // theirs, so an aggregator's multi-market export still reads correctly.
