...
def list_flagged_numbers(
    country: str | None = Query(default=None, description="Filter to one market."),
    store: Store = Depends(get_store),
) -> list[NumberReputationResponse]:
    """All numbers with at least one report, ranked by report count. Used by the admin
    dashboard's flagged-number review queue."""
    numbers = store.list_number_reputations()
    if country:
        code = country.strip().upper()
        # Match on where it was *reported*, not its home country, so a Nigerian
        # number running a scam into Ghana still appears in Ghana's queue.
        numbers = [n for n in numbers if code in n.countries]
    return [
        NumberReputationResponse(
            msisdn=n.msisdn, country=n.country, report_count=n.report_count,
            categories=n.categories, countries=n.countries,
...
def get_number_reputation(
    msisdn: str,
    country: str | None = Query(
        default=None,
        description="Needed only for a local-format number (0712345678), which is "
                    "ambiguous across markets. Ignored for international format.",
    ),
    store: Store = Depends(get_store),
) -> NumberReputationResponse:
    try:
        return lookup_number(store, msisdn, country)
    except (ValidationError, UnknownCountryError) as exc:
...
def list_reports(
    ...
    if country:
        code = country.strip().upper()
        reports = [r for r in reports if r.country == code]
    if region:
        reports = [r for r in reports if r.region == region]
    return [
        ReportListItem(
            id=r.id, msisdn=r.msisdn, country=r.country, category=r.category, region=r.region,
